...
public class PdfView {
    ...
    public static void createWebPrintJob(Activity activity, WebView webView, File directory, String fileName, final Callback callback) {
        ...
        String jobName = activity.getString(R.string.app_name) + "_Document";
    ...
    public static void openImageFile(final Activity activity, String title, String message, final String path){
        ...
        builder.setPositiveButton("Open", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                dialog.dismiss();
                fileChooserForImage(activity,path);
            }
        });
    ...
    private  static void fileChooserForImage(Activity activity, String path) {
        File file = new File(path);
        Intent target = new Intent("android.intent.action.VIEW");
        Uri uri = FileProvider.getUriForFile(activity, activity.getPackageName()+".fileprovider", file);
